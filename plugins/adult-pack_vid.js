...
if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*[❗𝐈𝐍𝐅𝐎❗] ESSE TIPO DE COMANDO ESTÁ DESATIVADO POR AQUI, Ô GLORIA. PARA ATIVAR, UTILIZE #enable modohorny*'
switch (command) {
case "pack": 
let url = await pack[Math.floor(Math.random() * pack.length)]
conn.sendButton(m.chat, `_🥵 Pack 🥵_`, author, url, [['🔄 PRÓXIMO, DEPRAVADO 🔄', `${usedPrefix + command}`]], m)
break
case "pack2":  
let url2 = await packgirl[Math.floor(Math.random() * packgirl.length)]
conn.sendButton(m.chat, `_🥵 Pack 2 🥵_`, author, url2, [['🔄 PRÓXIMO, DEPRAVADO 🔄', `${usedPrefix + command}`]], m) 
break
case "packh":
let url3 = await packmen[Math.floor(Math.random() * packmen.length)]
conn.sendButton(m.chat, `_🥵 Pack 3 🥵_`, author, url3, [['🔄 PRÓXIMO, DEPRAVADO 🔄', `${usedPrefix + command}`]], m) 
break
case "videox": case "videox":    
let url4 = await videosxxxc[Math.floor(Math.random() * videosxxxc.length)] 
await conn.sendButton(m.chat, `*ᴅɪsғʀᴜᴛᴀ ᴅᴇʟ ᴠɪᴅᴇᴏ 🥵*`, author, url4, [['🔄 PRÓXIMO, DEPRAVADO 🔄', `${usedPrefix + command}`]], m)  
break  
...
await conn.sendButton(m.chat, `*ᴅɪsғʀᴜᴛᴀ ᴅᴇʟ ᴠɪᴅᴇᴏ 🥵*`, author, url5, [['🔄 PRÓXIMO, DEPRAVADO 🔄', `${usedPrefix + command}`]], m)    
break    
}}
