...
if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*[❗𝐈𝐍𝐅𝐎❗] ESSE TIPO DE COMANDO ESTÁ DESATIVADO AQUI, PEÇA A UM ADM PARA UTILIZAR #enable modohorny*'
...
conn.sendButton(m.chat, `_${command}_`.trim(), author, haha, [['🔄 PRÓXIMO, DEPRAVADO. 🔄', `/${command}`]], m)}
...
conn.sendButton(m.chat, `_${command}_`.trim(), author, res, [['🔄 PRÓXIMO, DEPRAVADO. 🔄', `/${command}`]], m)}
...
conn.sendButton(m.chat, `_${command}_`.trim(), author, url, [['🔄 PRÓXIMO, DEPRAVADO. 🔄', `/${command}`]], m)}
